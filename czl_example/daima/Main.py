...
dataset_names = ['IP','SA','PU','RV','FM']
parser = argparse.ArgumentParser(description="Run deep learning experiments on"
                                             " various hyperspectral datasets")
parser.add_argument('--dataset', type=str, default='RV', choices=dataset_names,
                    help="Dataset to use.")

parser.add_argument('--train',type=int, default=1,choices=(0,1))
parser.add_argument('--train_VAE',type=int, default=1,choices=(0,1))
parser.add_argument('--train_AAE',type=int, default=1,choices=(0,1))
parser.add_argument('--train_contrast',type=int, default=1,choices=(0,1))
parser.add_argument('--VAE_epoch',type=int, default=10)
parser.add_argument('--AAE_epoch',type=int, default=10)
parser.add_argument('--perclass', type=float, default=20) #会除以100
parser.add_argument('--device', type=str, default="cuda:0", choices=("cuda:0","cuda:1"))
parser.add_argument('--projection_dim', type=int, default=128)
parser.add_argument('--pro_dim', type=int, default=2048)
...
parser.add_argument('--Patch_channel', type=int, default=15) ## PCA 之后的维数 ori = 15
...
# if args.dataset == 'RV' or args.dataset == 'FM'：
output_units = 2

Datadir='./DataArray/'
X1Path = Datadir + 'X1.npy'
X2Path = Datadir + 'X2.npy'
yPath = Datadir + 'y.npy'
# # 2. 生成两个自编码器并训练，保存好模型参数
train_start = time.time()
if args.train:
    Preprocess(X1Path, X2Path,yPath, args.dataset, args.Windowsize, Patch_channel=args.Patch_channel)  # args.Patch_channel
    if args.train_VAE:
        TrainVAE_patch(X1Path, X2Path, Patch_channel=args.Patch_channel, windowSize=args.Windowsize, encoded_dim=args.encoded_dim,
                      batch_size=args.batch_size, epoch=args.VAE_epoch)
    if args.train_AAE:
        TrainAAE_patch(X1Path, X2Path ,Patch_channel=args.Patch_channel,windowSize=args.Windowsize,encoded_dim=args.encoded_dim,batch_size=args.batch_size,epoch=args.AAE_epoch)
...
VAEFeatures=SaveFeatures_VAE(X1Path, X2Path, Patch_channel=args.Patch_channel,windowSize=args.Windowsize,encoded_dim=args.encoded_dim,batch_size=args.batch_size)
np.save(VAEPath, VAEFeatures)
AAEFeatures=SaveFeatures_AAE(X1Path, X2Path, Patch_channel=args.Patch_channel,windowSize=args.Windowsize,encoded_dim=args.encoded_dim,batch_size=args.batch_size)
...
for feature in ["Contrast","AAE","VAE"]:
    ...
    if args.classifier=='svm':
        ...
        train_stop2 = time.time()
        test_start3 = time.time()
        Predictions=TestNN(n_features=projection_dim,n_classes=output_units, ModelPath=ModelPath,Datadir=Datadir,feature=feature)
        test_stop3 = time.time()
#####算上训练分类器和测试分类器的时间V
...
######不计分类器的训练和测试时间损耗
    ...
    ytest=np.load(Datadir+'y.npy')

    if feature == "Contrast":
        np.save(Datadir + 'Contrast_predict.npy', Predictions)
    elif feature == "AAE":
        np.save(Datadir + 'AAE_predict.npy', Predictions)
    elif feature == "VAE":
        np.save(Datadir + 'VAE_predict.npy', Predictions)
    ...
    add_info=[args.dataset,args.perclass,args.temperature,args.Windowsize,args.classifier,feature,args.VAE_epoch, args.AAE_epoch,oa,aa,kappa,TrainTime,TestTime]+each_acc_str.split('[')[0].split(']')[0].split(',')
