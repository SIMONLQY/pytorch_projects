...
def test(model,test_loader,loss_f,test_size):
    ...
    correct = 0
    with torch.no_grad():
        for i, (x,y,input_length) in enumerate(test_loader):
            x = x.cuda()
            y = y.cuda()
            input_length = input_length.cuda()
            optimizer.zero_grad()
            y_hat = model(x,input_length)
...
if __name__ == "__main__":
    ...
    # 数据准备
    full_dataset = my_dataset.MyDataset('../SAPractice/train/', embedding, word2id)
    ...
    if gpu:
        torch.distributed.init_process_group(backend='nccl', init_method='tcp://localhost:23458', rank=0, world_size=1)
        model = model.cuda()
        model = DDP(model)
        loss_f = loss_f.cuda()
    if load_flag:
        ...
        model.load_state_dict(torch.load('./model/my_model_bi.pth')['model'])
    ...
    for epoch in range(epoch_num):
        for i, (x, y,input_length) in enumerate(train_loader):
            # 如果使用gpu，数据要转换一下
            if gpu:
                x, y = x.cuda(), y.cuda()
                input_length = input_length.cuda()
